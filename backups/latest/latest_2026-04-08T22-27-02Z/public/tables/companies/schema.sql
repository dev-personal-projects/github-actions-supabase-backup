...
\restrict Oc8OSgK3aPadk8UwTIE3JCIaZa4Zbb4T4oL0XWKAo55FOkaCrbWHcNgnGvNqsv9
...
CREATE TABLE public.companies (
    ...
    cmp_country_iso2 character(2),
    CONSTRAINT ck_companies__cmp_country_iso2 CHECK (((cmp_country_iso2 IS NULL) OR ((btrim((cmp_country_iso2)::text) ~ '^[A-Z]{2}$'::text) AND (btrim((cmp_country_iso2)::text) = upper(btrim((cmp_country_iso2)::text))))))
);


--
-- Name: CONSTRAINT ck_companies__cmp_country_iso2 ON companies; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON CONSTRAINT ck_companies__cmp_country_iso2 ON public.companies IS 'Enforce uppercase ISO-2 in cmp_country_iso2 when present (staged rollout; cmp_country remains human-readable).';
...
\unrestrict Oc8OSgK3aPadk8UwTIE3JCIaZa4Zbb4T4oL0XWKAo55FOkaCrbWHcNgnGvNqsv9
