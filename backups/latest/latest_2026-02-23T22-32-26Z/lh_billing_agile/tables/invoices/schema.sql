...
\restrict VY7igWbkdi2Lb8g8BCd8p9nZGOkcY8jMlVNbRty2nZNkDtDfbt13ehk1EgrKPNy
...
CREATE TABLE lh_billing_agile.invoices (
    ...
    remittance_profile_id bigint,
    remittance_details jsonb,
    remittance_resolved_at timestamp with time zone,
    ...
    pdf_render_attempt_count integer DEFAULT 0 NOT NULL,
    pdf_render_next_attempt_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    pdf_render_last_error text,
    pdf_render_last_failed_at timestamp with time zone,
    pdf_render_blocked boolean DEFAULT false NOT NULL,
    pdf_render_blocked_reason text,
    ...
    CONSTRAINT ck_invoices__pdf_render_blocked_reason_required CHECK (((pdf_render_blocked = false) OR (btrim(COALESCE(pdf_render_blocked_reason, ''::text)) <> ''::text))),
    CONSTRAINT ck_invoices__period_valid CHECK ((period_end >= period_start)),
    CONSTRAINT ck_invoices__remittance_snapshot_consistent CHECK ((((remittance_profile_id IS NULL) AND (remittance_details IS NULL) AND (remittance_resolved_at IS NULL)) OR ((remittance_profile_id IS NOT NULL) AND (remittance_details IS NOT NULL) AND (remittance_resolved_at IS NOT NULL)))),
    CONSTRAINT ck_invoices__status CHECK ((status = ANY (ARRAY['draft'::text, 'ready_for_pdf'::text, 'generated'::text, 'failed_generate'::text, 'pdf_generated'::text, 'failed_pdf_generate'::text, 'postprocessing_queued'::text, 'failed_to_queue_postprocessing'::text, 'postprocessing'::text, 'postprocessing_failed'::text, 'ready_to_send'::text, 'sent'::text, 'failed_send'::text, 'void'::text]))),
    CONSTRAINT ck_invoices__test_recipients_dry_run_only CHECK (((is_dry_run = true) OR (test_recipients IS NULL) OR (cardinality(test_recipients) = 0))),
...
--
-- Name: COLUMN invoices.remittance_profile_id; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoices.remittance_profile_id IS 'Resolved remittance profile used for PDF generation (FK to lh_billing_system.remittance_profiles).';


--
-- Name: COLUMN invoices.remittance_details; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoices.remittance_details IS 'Snapshot of remittance payload used to render the invoice PDF (JSON).';


--
-- Name: COLUMN invoices.remittance_resolved_at; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoices.remittance_resolved_at IS 'Timestamp when remittance_details was snapshotted onto the invoice.';
...
COMMENT ON COLUMN lh_billing_agile.invoices.status IS 'Lifecycle status (includes Phase 3 postprocessing states). See PRD Agile Postprocessor for authoritative meanings.';
...
--
-- Name: COLUMN invoices.pdf_render_attempt_count; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoices.pdf_render_attempt_count IS 'Number of Phase 2 (PDF render + checkpoint) attempts since last successful checkpoint.';


--
-- Name: COLUMN invoices.pdf_render_next_attempt_at; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoices.pdf_render_next_attempt_at IS 'Next eligible time for Phase 2 retry (backoff).';


--
-- Name: COLUMN invoices.pdf_render_last_error; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoices.pdf_render_last_error IS 'Most recent Phase 2 error message (sanitized/compact).';


--
-- Name: COLUMN invoices.pdf_render_last_failed_at; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoices.pdf_render_last_failed_at IS 'Timestamp of the most recent Phase 2 failure.';


--
-- Name: COLUMN invoices.pdf_render_blocked; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoices.pdf_render_blocked IS 'True when Phase 2 is quarantined after repeated failures (operator intervention required).';


--
-- Name: COLUMN invoices.pdf_render_blocked_reason; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoices.pdf_render_blocked_reason IS 'Reason for Phase 2 quarantine when pdf_render_blocked=true.';
...
--
-- Name: ix_invoices__pdf_render_polling; Type: INDEX; Schema: lh_billing_agile; Owner: -
--

CREATE INDEX ix_invoices__pdf_render_polling ON lh_billing_agile.invoices USING btree (status, pdf_render_blocked, pdf_render_next_attempt_at, invoice_id) WHERE (status = ANY (ARRAY['ready_for_pdf'::text, 'failed_pdf_generate'::text]));
...
--
-- Name: invoices fk_invoices__remittance_profiles; Type: FK CONSTRAINT; Schema: lh_billing_agile; Owner: -
--

ALTER TABLE ONLY lh_billing_agile.invoices
    ADD CONSTRAINT fk_invoices__remittance_profiles FOREIGN KEY (remittance_profile_id) REFERENCES lh_billing_system.remittance_profiles(remittance_profile_id) ON DELETE RESTRICT;
...
\unrestrict VY7igWbkdi2Lb8g8BCd8p9nZGOkcY8jMlVNbRty2nZNkDtDfbt13ehk1EgrKPNy
