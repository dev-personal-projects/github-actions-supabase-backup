...
\restrict JBgNaEQlVR2dfv31EjYg6zxa1HdPRXVMcUMRwPb03xpzbfbzYmIvjCljWOhtbBl
...
COMMENT ON COLUMN lh_billing_agile.invoices.pdf_render_attempt_count IS 'Number of Phase 2 (PDF render + queue) attempts since last successful checkpoint.';
...
CREATE INDEX ix_invoices__pdf_render_polling ON lh_billing_agile.invoices USING btree (status, pdf_render_blocked, pdf_render_next_attempt_at, invoice_id) WHERE (status = 'ready_for_pdf'::text);
...
\unrestrict JBgNaEQlVR2dfv31EjYg6zxa1HdPRXVMcUMRwPb03xpzbfbzYmIvjCljWOhtbBl
