...
\restrict H7rvqrkHmTzrlLMHHvJXHd7TzRsTgjZBtrY5IwcgzqkDcxL3wZ6siFTvp0Ierj1
...
CREATE TABLE lh_billing_agile.invoice_delivery_outbox (
    ...
    invoice_is_dry_run boolean DEFAULT false NOT NULL,
    ...
    graph_message_created_at timestamp with time zone,
    graph_message_sent_at timestamp with time zone,
    ...
    CONSTRAINT ck_invoice_delivery_outbox__correlation_id_bounded CHECK ((char_length(correlation_id) <= 128)),
    CONSTRAINT ck_invoice_delivery_outbox__correlation_id_not_blank CHECK ((btrim(correlation_id) <> ''::text)),
    CONSTRAINT ck_invoice_delivery_outbox__graph_message_id_not_blank CHECK (((graph_message_id IS NULL) OR (btrim(graph_message_id) <> ''::text))),
...
--
-- Name: COLUMN invoice_delivery_outbox.invoice_is_dry_run; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoice_delivery_outbox.invoice_is_dry_run IS 'Snapshot of invoices.is_dry_run at enqueue time (sender uses this to choose blob container deterministically)';
...
--
-- Name: COLUMN invoice_delivery_outbox.graph_message_id; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoice_delivery_outbox.graph_message_id IS 'Graph message id persisted for idempotent send/resume (prefer immutable id)';


--
-- Name: COLUMN invoice_delivery_outbox.graph_message_created_at; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoice_delivery_outbox.graph_message_created_at IS 'Timestamp when graph_message_id was first persisted (draft created)';


--
-- Name: COLUMN invoice_delivery_outbox.graph_message_sent_at; Type: COMMENT; Schema: lh_billing_agile; Owner: -
--

COMMENT ON COLUMN lh_billing_agile.invoice_delivery_outbox.graph_message_sent_at IS 'Timestamp when Graph send was confirmed (may differ from DB sent_at)';
...
--
-- Name: ix_invoice_delivery_outbox__graph_message_id; Type: INDEX; Schema: lh_billing_agile; Owner: -
--

CREATE INDEX ix_invoice_delivery_outbox__graph_message_id ON lh_billing_agile.invoice_delivery_outbox USING btree (graph_message_id) WHERE (graph_message_id IS NOT NULL);
...
\unrestrict H7rvqrkHmTzrlLMHHvJXHd7TzRsTgjZBtrY5IwcgzqkDcxL3wZ6siFTvp0Ierj1
